...
| `profile [--runs N] [--top N] [--json] [--no-tui]` | `run_profile()` | `nvim --headless --startuptime` を N 回 (デフォルト 3) 実行して startup 時間をプラグイン単位に集計。sourcing/require 行を parse → path prefix match で `<repos_dir>/<canonical>` に帰属、`<merged_dir>/...` は `[merged]` 擬似グループに集約。TUI (デフォルト) / `--json` / `--no-tui` の 3 出力。self_ms (子を含まない) を主指標にして二重計上を避ける |
