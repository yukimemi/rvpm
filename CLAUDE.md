...
| `tune [query] [--ai <backend>] [--no-ai]` | `run_tune()` | **既に config に登録済みの** プラグインに対して AI chat loop (`run_ai_tune`) を回す。`add --ai` との違いは clone を skip + 既存 entry を AI に見せて「改善して」と頼むこと。Apply 時は `replace_plugin_entry_with_ai_toml(.., preserved_keys=&[])` で **全 field を AI 提案で上書き**。user 側の guardrail は Chat ループ中に "X は触らないで" と AI に伝える運用。AI 専用 — `effective_ai == Off` なら明示 error (非 AI tweak は `set` で代替) |
