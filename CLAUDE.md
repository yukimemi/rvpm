...
- **Monitoring stop conditions**:
  1. **All open threads have settled** → the PR is quiet. The moment both PRs (or all target PRs) become quiet, exit the loop and ask the owner for a merge decision. If the bot acks quickly, there is no need to wait 30 minutes.
...
- Malformed / missing files emit a warn on stderr and fall back to an empty
  LockFile (resilience).
...
- `chezmoi::write_path` / `chezmoi::apply` are implemented as **async + 2s
  ...
  accumulate into something orders of magnitude larger). On timeout, a warn
...
Resilience: if `nvim` is not on PATH, only a warn is emitted and rvpm continues. Even if the nvim process exits non-zero, Ok is returned. Duplicate-tag warnings from `:helptags` (E154 etc.) are passed through to stderr — they carry value as an improvement signal for users who explicitly opt into merge, so they are not suppressed.
...
| `doctor` | `run_doctor()` | One-shot command that diagnoses 16 items across config / state / Neovim integration / external tools. 4 categories (plugin config / state integrity / Neovim integration / external tools); output respects `options.icons` (nerd/unicode/ascii). Exit codes: `0` = all ok, `1` = errors present, `2` = warns only. External commands (nvim/git/chezmoi) are probed via `tokio::process::Command` + 2s timeout so they cannot hang. |
