...
| ファイル | Phase | 実行タイミング |
|---|---|---|
| `~/.config/rvpm/before.lua` | 3 | `load_lazy` helper 定義後、全プラグインの `init.lua` より前 |
| `~/.config/rvpm/after.lua` | 9 | 全 lazy trigger 登録後 |
...
Phase 3:   global before.lua                   ← ~/.config/rvpm/before.lua (存在する場合)
...
Phase 9:   global after.lua                   ← ~/.config/rvpm/after.lua (存在する場合)
```
...
| `~/.config/rvpm/before.lua` | グローバル before hook (phase 3、全 init.lua より前。存在すれば自動適用) |
| `~/.config/rvpm/after.lua` | グローバル after hook (phase 9、全 lazy trigger 登録後。存在すれば自動適用) |
