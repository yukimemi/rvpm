...
- **カラースキームの自動検出**: lazy プラグインのクローン先に `colors/*.{vim,lua}` が存在する場合、`generate_loader()` が generate 時にそのファイル名を走査し、`ColorSchemePre` autocmd ハンドラを Phase 4.3 として自動生成する。設定ファイルへの追記は不要。eager プラグインは `colors/` が既に RTP 上にあるため影響を受けない。
