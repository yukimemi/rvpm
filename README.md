...
| `theme` | table | built-in defaults | Customize colors across the `sync` / `update` / `list` / `browse` TUIs via `[options.theme]` (14 semantic fields: `foreground`, `background`, `success`, `error`, `accent`, …). Values accept color names, `"#RRGGBB"`, or 0-255 palette indices; invalid values warn on stderr and fall back per-field. Full field list and example in [docs/cli.md → TUI theme](docs/cli.md#tui-theme) |
