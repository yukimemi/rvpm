...
| `theme` | table | built-in defaults | Customize colors across the `sync` / `update` / `list` / `browse` TUIs via `[options.theme]` (14 semantic fields: `foreground`, `background`, `success`, `error`, `accent`, …). Values accept color names, `"#RRGGBB"`, or 0-255 palette indices; invalid values warn on stderr and fall back per-field. Named presets (built-in + `[options.theme_presets.<name>]` custom ones) are selectable with live preview via `T` in `rvpm list`. Full field list, presets, and example in [docs/cli.md → TUI theme](docs/cli.md#tui-theme) |
...
| `t` | AI tune (refine) the selected plugin's config |
| `T` | Pick a theme preset with live preview (`Enter` asks to confirm, `Enter`/`y` again applies + persists, `Esc` cancels) — see [TUI theme](docs/cli.md#tui-theme) |
| `S` | Sync all plugins |
