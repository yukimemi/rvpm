...
// All `run_*` subcommand handlers now live in `commands` (#217); the dispatcher
// in `run_cli` calls them by bare name via this glob.
use crate::commands::*;
...
mod tests {
    use super::*;
    // These helpers moved to dedicated modules (#217); the test cases here still
    // reference them by bare name, so glob them in until the test blocks are
    // relocated next to their modules (tracked in #224).
