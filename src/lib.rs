...
// Merge-mode classification + view/merged tree building now live in `merge`
// (#217). Glob-import for the internal call sites; re-export the public
// `PluginMergeMode` enum so `rvpm::PluginMergeMode` stays a valid path.
...
async fn run_generate(force: bool) -> Result<()> {
