...
- `before.lua` — runs **after** RTP append but **before** the plugin's `plugin/*` script is sourced. Use this **only** when the README explicitly says something must run before the plugin's `plugin/*` registers its commands / autocmds / handlers (e.g. "call `require('foo').register(...)` before the plugin is sourced", or "this option must be set before the plugin's `plugin/*` runs"). Most plugins do **not** need before.lua. **Don't put `vim.g.<name> = ...` here — that belongs in `init.lua`.**
