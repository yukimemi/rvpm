...
# setup() call. The **presence** of `setup` is the switch that makes rvpm call
# `require("<module>").setup(<opts>)` for this plugin.
setup = {}            # `setup = {}` means "call setup with no options". Omit the field entirely and rvpm never calls setup.
# setup = { throttle = 500, notify = true, cursorline = { enable = true } }   # the table IS the options; nested tables are fine
# setup = { main = "mini.pick", opts = { n = 7 } }   # descriptor form: name the module to require, options under `opts`.
#                      # Use it ONLY when module resolution fails (rvpm warns) or picks the wrong module — rvpm normally
#                      # resolves the module from the plugin's `lua/` tree at generate time. A table whose keys are only
#                      # `main` / `opts` is read as this descriptor, so pass options literally named `main` as
#                      # `setup = { opts = { main = ... } }`.
...
- `after.lua` — runs at **plugin load time**, **after** the plugin's `plugin/*` is sourced **and after the `setup` call**. This is the place for anything `setup` cannot express: Lua functions/callbacks, `vim.keymap.set`, autocmds, `vim.*` calls, conditionals — i.e. statements rather than data.

**Default rule for `setup({...})`: use the `setup` field, not `after.lua`.** Decide by looking at the *values* the README passes to setup:

- **Data only** (strings, numbers, booleans, arrays, nested tables) → put them in `setup` and write **no** `after.lua`. `setup = {}` covers the common "just call `require('foo').setup({})`" case.
- **Needs a Lua function / callback / `vim.*` call / extra statements** (keymaps, autocmds, `on_attach = function() ... end`, computed values) → write `after.lua`. TOML cannot hold Lua functions, so this is the escape hatch.
- **One setup call that needs both data and Lua** (e.g. `setup({ enabled = true, on_attach = function() … end })`) → keep the **whole** call in `after.lua` and omit `setup`. Do **not** split one call: `setup` and the hook would both have to set up the same module, which rvpm warns about, and the callback has nowhere else to go. Split only when the plugin documents a *separate* post-setup API (`require('x').add_rule(…)`, `:h x-api`) — then the data goes in `setup` and only the extra calls in `after.lua`, which is also why rvpm runs **`setup` → `after.lua`**.
- **Never call setup in both places.** If `setup` is present, `after.lua` (and `before.lua`) must not contain a `.setup(` call for that same module — rvpm warns about the double setup. Setting up a *different* module from a hook (e.g. another module of the same monorepo) is fine and stays silent.
- **Ordering constraint from the README** ("this must run before the plugin's `plugin/*`" / "call this before the plugin is sourced") → `before.lua`, as always. The `setup` call happens after `plugin/*`, so it is not an option there.
...
4. **For `[[plugins]]` entry merged**: keep `name`, `url`, `rev`, `dev`, `dst`, `cond`, `when`, `build`, `build_lua`, `depends`, `setup` intact unless you have a strong reason to change them. `on_*` triggers are where you'd typically refine. In particular, **never discard values the user hand-wrote in `setup`** — merge your additions into their table and keep their keys; and keep a `main` they spelled out in the descriptor form (it exists because module resolution needed help).
5. **Fold a data-only `after.lua` into `setup`.** If the existing `after.lua` contains nothing but a single setup call whose values are plain data (`require('foo').setup({ a = 1, b = "x" })`), move those values into `setup` in the `[[plugins]]` entry and output `(none)` for `<rvpm:after_lua>` / `<rvpm:after_lua_merged>` — the user is offered a "Remove existing after.lua" choice, so this collapses the hook file. Do **not** fold when the body has Lua functions, `vim.*` calls, keymaps, autocmds, comments worth keeping, or any statement besides that one setup call: those cannot be expressed in TOML, so leave them in `after.lua` and don't add `setup`.
...
- `setup` values are converted to a Lua table literal at generate time, so they must be expressible in TOML — never put a Lua function, `vim.*` call, or other Lua expression in `setup` (use `after.lua` for those).
